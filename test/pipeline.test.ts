...
describe('脏队列状态机', () => {
  test('创建即入队，processBatch 消费后清零', async () => {
    ...
    // 向量真实写入
    const emb = (env.repo as any).db.query('SELECT embedding FROM chunks LIMIT 1').get() as { embedding: string }
    expect(emb.embedding).not.toBeNull()
    expect(JSON.parse(emb.embedding).length).toBe(8) // FakeEmbedder 8 维
  })
